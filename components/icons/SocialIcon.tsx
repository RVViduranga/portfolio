...
import type { SocialKind } from "@/data/site";

interface IconProps {
  className?: string;
}

// Lucide no longer ships brand marks, so the two logos are inlined here.
...
export function SocialIcon({ kind, className = "size-4" }: IconProps & { kind: SocialKind }) {
  switch (kind) {
    case "github":
